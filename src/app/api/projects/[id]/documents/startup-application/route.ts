...
export async function POST(
  ...
  try {
    ...
    await deductCredits(user.id, await getPromptCreditCost('doc_startup_application'), 'ai_doc_startup_application', id)
    ...
    const { data: existingDoc } = await supabase
      ...
      .eq('project_id', id)
      .eq('type', 'startup_application')
      .order('created_at', { ascending: false })
      .limit(1)
      .single()

    if (existingDoc?.is_confirmed) {
      return errorResponse('이미 확정된 모두의 창업 신청서가 있습니다.', 400)
    }
    ...
    if (!prompt) {
      return errorResponse('모두의 창업 신청서 프롬프트를 찾을 수 없습니다.', 500)
    }
